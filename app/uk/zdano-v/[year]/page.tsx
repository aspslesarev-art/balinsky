...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `/uk/zdano-v/${year}`,
      languages: {
        ru: `${SITE_URL}/ru/sdano/${year}`,
        uk: `${SITE_URL}/uk/zdano-v/${year}`,
...
export default async function Page({ params }: { params: Params }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Residential complexes', item: `${SITE_URL}/uk/kompleksy` },
      { '@type': 'ListItem', position: 3, name: `Completed ${year}`, item: `${SITE_URL}/uk/zdano-v/${year}` },
    ],
  }
  ...
  return (
    <>
      ...
      <PageContainer>
        <Breadcrumbs items={[
          { label: 'Home', href: '/uk' },
          { label: 'Residential complexes', href: '/uk/kompleksy' },
        ...
        {matched.length === 0 ? (
          <div className="py-16 text-center text-[var(--color-text-muted)]">
            No matching complexes yet. Browse the <Link href="/uk/kompleksy" className="text-[var(--color-primary)] no-underline hover:underline">full catalogue</Link>.
          </div>
        ) : (
        ...
        <section className="mt-12 mb-8 max-w-3xl">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {[...VALID_YEARS].filter(y => y !== year).slice(0, 6).map(y => (
              <li key={y}>
                <Link href={`/uk/zdano-v/${y}`} className="inline-flex items-center gap-2 px-3 py-2.5 rounded-xl border border-[var(--color-border)] bg-white text-[#111827] no-underline hover:border-[var(--color-primary)] transition-colors">
                  Complexes completed in {y}
                </Link>
              </li>
            ))}
            <li>
              <Link href="/uk/investytsiyi-nerukhomist-bali" className="inline-flex items-center gap-2 px-3 py-2.5 rounded-xl border border-[var(--color-border)] bg-white text-[#111827] no-underline hover:border-[var(--color-primary)] transition-colors">
