...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataLoc(f, 'fr', { canonicalPath: '/fr/villas', noIndex: true })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  return (
    <>
      ...
      <PageContainer>
        <h1 className="pt-8 mb-2 text-[28px] md:text-[36px] font-semibold tracking-tight text-[#111827]">{buildHeadingLoc(filters, 'fr')}</h1>
        <div className="text-[14px] text-[var(--color-text-muted)] mb-6">{totalPoints} objets sur la carte{totalPoints !== groups.length && ` · ${groups.length} points`}</div>

        <CatalogTabs active="map" listHref={buildListHref(filters, 'fr')} mapHref={buildMapHref(filters, 'fr')} lang="fr" />

        <div className="mt-6">
          <VillaCatalogSearchBar initial={filters.q} current={filters} view="map" placeholder="Rechercher villas, quartiers, promoteurs…" />
        </div>

        <div className="mt-4">
          <VillaFiltersBar state={filters} options={options} view="map" lang="fr" />
        </div>

        <div className="mt-6">
          <VillasMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="fr" />
        </div>

        <VillasSeoContent filters={filters} variant="map" lang="fr" />
