...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      type: 'article',
      url: `/fr/investissement-immobilier-bali/${district}`,
    },
...
export default async function Page({ params }: { params: Params }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Bali property investment', item: `${SITE_URL}/fr/investissement-immobilier-bali` },
      { '@type': 'ListItem', position: 3, name: copy.name, item: `${SITE_URL}/fr/investissement-immobilier-bali/${district}` },
    ],
  }
  ...
  return (
    <>
      ...
      <PageContainer>
        <Breadcrumbs items={[
          { label: 'Home', href: '/fr' },
          { label: 'Bali property investment', href: '/fr/investissement-immobilier-bali' },
          { label: copy.name },
        ]} />
        ...
        <article className="mt-6 mb-16 max-w-4xl">
          ...
          <section className="mb-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Link href={`/fr/villas/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/fr/appartements/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/fr/residences/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
          ...
          <section className="mb-10 flex items-center gap-2 text-[14px] text-[var(--color-text-muted)]">
            <MapPin size={14} />
            See the district on map — <Link href={`/fr/villas/${district}/karta`} className="text-[var(--color-primary)] no-underline hover:underline">villa map</Link>
