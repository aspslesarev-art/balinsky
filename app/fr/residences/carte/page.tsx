...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataEn(f, { canonicalPath: '/fr/residences', noIndex: true })
  meta.title = `Carte · ${buildHeadingLocalized(f, 'fr')} | Balinsky`
  return meta
}
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  const options = buildOptions(enriched, filters, 'fr')
  ...
  for (const e of filtered) {
    ...
    seenSlug.add(e.slug)
    const types = e.types.length > 0 ? e.types.map(t => facetLabel('type', t, 'fr')).join(', ') : null
  ...
  return (
    <>
      ...
      <PageContainer>
        <h1 className="pt-8 mb-2 text-[28px] md:text-[36px] font-semibold tracking-tight text-[#111827]">{`Carte · ${buildHeadingLocalized(filters, 'fr')}`}</h1>
        <div className="text-[14px] text-[var(--color-text-muted)] mb-6">{totalPoints} résidences sur la carte{totalPoints !== groups.length && ` · ${groups.length} points`}</div>

        <CatalogTabs active="map" listHref={buildListHref(filters, 'fr')} mapHref={buildMapHref(filters, 'fr')} lang="fr" />

        <div className="mt-6">
          <ComplexCatalogSearchBar initial={filters.q} current={filters} view="map" placeholder="Rechercher résidences, régions, promoteurs…" />
        </div>

        <div className="mt-4">
          <ComplexFiltersBar state={filters} options={options} view="map" lang="fr" />
        </div>

        <div className="mt-6">
          <ComplexesMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="fr" />
        </div>

        <ComplexesSeoContent filters={filters} variant="map" lang="fr" />
