// English mirror of /ru/zhilye-kompleksy. Same component, just lang='fr'
...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataEn(f, {
    canonicalPath: '/fr/residences',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'complexes', locale: 'fr', ...villaCategoryStats((await loadAll()).enriched) })
