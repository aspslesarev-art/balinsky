...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataLoc(f, 'id', {
    canonicalPath: '/id/apartemen',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'apartments', locale: 'id', ...apartmentCategoryStats((await loadAll()).enriched) })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  const localCanonical = canonical ? hubPath(canonical, 'id') : null
  if (localCanonical && canonical !== '/ru/apartamenty' && hasAnyFilter(filters)) {
    permanentRedirect(localCanonical)
  }
  return <ApartamentyCatalog filters={filters} page={1} basePath="/id/apartemen" lang="id" />
}
