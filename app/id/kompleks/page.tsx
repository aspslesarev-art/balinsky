// English mirror of /ru/zhilye-kompleksy. Same component, just lang='id'
...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataEn(f, {
    canonicalPath: '/id/kompleks',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'complexes', locale: 'id', ...villaCategoryStats((await loadAll()).enriched) })
