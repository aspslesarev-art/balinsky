...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const copy = getDistrictCopy(district, 'id')
  ...
  return {
    ...
    alternates: {
      canonical: `/id/investasi-properti-bali/${district}`,
      languages: {
        ru: `${SITE_URL}/ru/investicii/${district}`,
        en: `${SITE_URL}/id/investasi-properti-bali/${district}`,
    ...
    openGraph: {
      ...
      type: 'article',
      url: `/id/investasi-properti-bali/${district}`,
    },
...
export default async function Page({ params }: { params: Params }) {
  ...
  const breadcrumbJsonLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: `${SITE_URL}/id` },
      { '@type': 'ListItem', position: 2, name: 'Bali property investment', item: `${SITE_URL}/id/investasi-properti-bali` },
      { '@type': 'ListItem', position: 3, name: copy.name, item: `${SITE_URL}/id/investasi-properti-bali/${district}` },
    ],
  }
  ...
  return (
    <>
      ...
      <PageContainer>
        <Breadcrumbs items={[
          { label: 'Home', href: '/id' },
          { label: 'Bali property investment', href: '/id/investasi-properti-bali' },
          { label: copy.name },
        ]} />
        ...
        <article className="mt-6 mb-16 max-w-4xl">
          ...
          <section className="mb-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Link href={`/id/vila/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/id/apartemen/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/id/kompleks/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
          ...
          <section className="mb-10 flex items-center gap-2 text-[14px] text-[var(--color-text-muted)]">
            <MapPin size={14} />
            See the district on map — <Link href={`/id/vila/${district}/karta`} className="text-[var(--color-primary)] no-underline hover:underline">villa map</Link>
