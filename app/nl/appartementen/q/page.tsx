...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataLoc(f, 'nl', {
    canonicalPath: '/nl/appartementen',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'apartments', locale: 'nl', ...apartmentCategoryStats((await loadAll()).enriched) })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  const localCanonical = canonical ? hubPath(canonical, 'nl') : null
  if (localCanonical && canonical !== '/ru/apartamenty' && hasAnyFilter(filters)) {
    permanentRedirect(localCanonical)
  }
  return <ApartamentyCatalog filters={filters} page={1} basePath="/nl/appartementen" lang="nl" />
}
