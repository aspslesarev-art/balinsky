  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...
        en: `${SITE_URL}/nl/opgeleverd-in/${year}`,
...
export default async function Page({ params }: { params: Params }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Residential complexes', item: `${SITE_URL}/nl/wooncomplexen` },
      { '@type': 'ListItem', position: 3, name: `Completed ${year}`, item: `${SITE_URL}/nl/opgeleverd-in/${year}` },
    ],
  }
  ...
  return (
    <>
      ...
      <PageContainer>
        <Breadcrumbs items={[
          { label: 'Home', href: '/nl' },
          { label: 'Residential complexes', href: '/nl/wooncomplexen' },
        ...
        {matched.length === 0 ? (
          <div className="py-16 text-center text-[var(--color-text-muted)]">
            No matching complexes yet. Browse the <Link href="/nl/wooncomplexen" className="text-[var(--color-primary)] no-underline hover:underline">full catalogue</Link>.
          </div>
        ) : (
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
            {matched.map(c => <ComplexCard key={c.id} c={c} lang="nl" />)}
          </div>
        )}
        ...
        <section className="mt-12 mb-8 max-w-3xl">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {[...VALID_YEARS].filter(y => y !== year).slice(0, 6).map(y => (
              <li key={y}>
                <Link href={`/nl/opgeleverd-in/${y}`} className="inline-flex items-center gap-2 px-3 py-2.5 rounded-xl border border-[var(--color-border)] bg-white text-[#111827] no-underline hover:border-[var(--color-primary)] transition-colors">
                  Complexes completed in {y}
                </Link>
              </li>
            ))}
            <li>
              <Link href="/nl/bali-vastgoed-investering" className="inline-flex items-center gap-2 px-3 py-2.5 rounded-xl border border-[var(--color-border)] bg-white text-[#111827] no-underline hover:border-[var(--color-primary)] transition-colors">
