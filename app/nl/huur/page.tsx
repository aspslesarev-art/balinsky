...
export async function generateMetadata() {
  const base = generateRentalListMetadata('nl')
  const cat = generateCategoryMeta({ category: 'rental', locale: 'nl', count: (await loadFreshRental('nl')).length })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  return <RentalListShell initial={parseRentalSP(sp)} lang="nl" />
}
