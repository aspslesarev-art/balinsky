...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataLoc(f, 'nl', { canonicalPath: '/nl/villas', noIndex: true })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  for (const e of filtered) {
    ...
    seenSlug.add(slug)
    const titleRaw =
      tField(e.data, 'SEO:Title', 'nl') ?? firstString(e.data['Имя ENG']) ?? firstString(e.data['Name']) ?? slug
  ...
  return (
    <>
      ...
      <PageContainer>
        <h1 className="pt-8 mb-2 text-[28px] md:text-[36px] font-semibold tracking-tight text-[#111827]">{buildHeadingLoc(filters, 'nl')}</h1>
        <div className="text-[14px] text-[var(--color-text-muted)] mb-6">{totalPoints} objecten op de kaart{totalPoints !== groups.length && ` · ${groups.length} punten`}</div>

        <CatalogTabs active="map" listHref={buildListHref(filters, 'nl')} mapHref={buildMapHref(filters, 'nl')} lang="nl" />

        <div className="mt-6">
          <VillaCatalogSearchBar initial={filters.q} current={filters} view="map" placeholder="Zoek villa's, districten, ontwikkelaars…" />
        </div>

        <div className="mt-4">
          <VillaFiltersBar state={filters} options={options} view="map" lang="nl" />
        </div>

        <div className="mt-6">
          <VillasMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="nl" />
        </div>

        <VillasSeoContent filters={filters} variant="map" lang="nl" />
