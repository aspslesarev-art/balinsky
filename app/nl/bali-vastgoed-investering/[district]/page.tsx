...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const copy = getDistrictCopy(district, 'nl')
  ...
  return {
    ...
    alternates: {
      canonical: `/nl/bali-vastgoed-investering/${district}`,
      languages: {
        ru: `${SITE_URL}/ru/investicii/${district}`,
        en: `${SITE_URL}/nl/bali-vastgoed-investering/${district}`,
    ...
    openGraph: {
      ...
      type: 'article',
      url: `/nl/bali-vastgoed-investering/${district}`,
    },
...
export default async function Page({ params }: { params: Params }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Bali property investment', item: `${SITE_URL}/nl/bali-vastgoed-investering` },
      { '@type': 'ListItem', position: 3, name: copy.name, item: `${SITE_URL}/nl/bali-vastgoed-investering/${district}` },
    ],
  }
  ...
  return (
    <>
      ...
      <PageContainer>
        <Breadcrumbs items={[
          { label: 'Home', href: '/nl' },
          { label: 'Bali property investment', href: '/nl/bali-vastgoed-investering' },
          { label: copy.name },
        ]} />
        ...
        <article className="mt-6 mb-16 max-w-4xl">
          ...
          <section className="mb-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Link href={`/nl/villas/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/nl/appartementen/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/nl/wooncomplexen/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
          ...
          <section className="mb-10 flex items-center gap-2 text-[14px] text-[var(--color-text-muted)]">
            <MapPin size={14} />
            See the district on map — <Link href={`/nl/villas/${district}/karta`} className="text-[var(--color-primary)] no-underline hover:underline">villa map</Link>
