...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataEn(f, { canonicalPath: '/nl/wooncomplexen', noIndex: true })
  meta.title = `Kaart · ${buildHeadingLocalized(f, 'nl')} | Balinsky`
  return meta
}
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  const options = buildOptions(enriched, filters, 'nl')
  ...
  for (const e of filtered) {
    ...
    seenSlug.add(e.slug)
    const types = e.types.length > 0 ? e.types.map(t => facetLabel('type', t, 'nl')).join(', ') : null
  ...
  return (
    <>
      ...
      <PageContainer>
        ...
        <div className="mt-6">
          <ComplexCatalogSearchBar initial={filters.q} current={filters} view="map" placeholder="Zoek wooncomplexen, regio's, ontwikkelaars…" />
        </div>

        <div className="mt-4">
          <ComplexFiltersBar state={filters} options={options} view="map" lang="nl" />
        </div>

        <div className="mt-6">
          <ComplexesMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="nl" />
        </div>

        <ComplexesSeoContent filters={filters} variant="map" lang="nl" />
