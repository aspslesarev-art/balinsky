// English mirror of /ru/zhilye-kompleksy. Same component, just lang='nl'
...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataEn(f, {
    canonicalPath: '/nl/wooncomplexen',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'complexes', locale: 'nl', ...villaCategoryStats((await loadAll()).enriched) })
