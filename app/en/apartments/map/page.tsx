...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataEn(f, { canonicalPath: '/en/apartments', noIndex: true })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  return (
    <>
      ...
      <PageContainer>
        <h1 className="pt-8 mb-2 text-[28px] md:text-[36px] font-semibold tracking-tight text-[#1A1A1A]">{buildHeadingLoc(filters, 'en')}</h1>
        ...
        <div className="mt-4">
          <FiltersBar state={filters} options={options} view="map" lang="en" />
        </div>
