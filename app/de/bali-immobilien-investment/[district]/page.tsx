...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const copy = getDistrictCopy(district, 'de')
  ...
  return {
    ...
    alternates: {
      canonical: `/de/bali-immobilien-investment/${district}`,
      languages: {
        ru: `${SITE_URL}/ru/investicii/${district}`,
        en: `${SITE_URL}/de/bali-immobilien-investment/${district}`,
    ...
    openGraph: {
      ...
      type: 'article',
      url: `/de/bali-immobilien-investment/${district}`,
    },
...
export default async function Page({ params }: { params: Params }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Bali property investment', item: `${SITE_URL}/de/bali-immobilien-investment` },
      { '@type': 'ListItem', position: 3, name: copy.name, item: `${SITE_URL}/de/bali-immobilien-investment/${district}` },
    ],
  }
  ...
  return (
    <>
      ...
      <PageContainer>
        <Breadcrumbs items={[
          { label: 'Home', href: '/de' },
          { label: 'Bali property investment', href: '/de/bali-immobilien-investment' },
          { label: copy.name },
        ]} />
        ...
        <article className="mt-6 mb-16 max-w-4xl">
          ...
          <section className="mb-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Link href={`/de/villen/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/de/apartments/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/de/wohnanlagen/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
          ...
          <section className="mb-10 flex items-center gap-2 text-[14px] text-[var(--color-text-muted)]">
            <MapPin size={14} />
            See the district on map — <Link href={`/de/villen/${district}/karta`} className="text-[var(--color-primary)] no-underline hover:underline">villa map</Link>
