...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataLoc(f, 'de', {
    canonicalPath: '/de/apartments',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'apartments', locale: 'de', ...apartmentCategoryStats((await loadAll()).enriched) })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  const localCanonical = canonical ? hubPath(canonical, 'de') : null
  if (localCanonical && canonical !== '/ru/apartamenty' && hasAnyFilter(filters)) {
    permanentRedirect(localCanonical)
  }
  return <ApartamentyCatalog filters={filters} page={1} basePath="/de/apartments" lang="de" />
}
