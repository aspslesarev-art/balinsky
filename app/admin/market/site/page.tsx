...
export default async function MarketSitePage() {
  ...
  return (
    <MarketShell>
      ...
      <Panel
        title="Нет на сайте"
        hint="Прайс застройщика открывает продажи новых юнитов, а карточки под них никто не заводит — раньше это ловилось только глазами, строка за строкой. Считаем различимыми предложениями: десять одинаковых студий в корпусе — это одна карточка, а не десять. «Завести» копирует карточку-образец того же комплекса (её фото, район, сроки, условия) и подставляет площадь, этаж, спальни и цену из прайса; метраж в текстах переписывается. Цена берётся с надбавкой комплекса — по ней уже живут соседние карточки. Пара с юнитом заводится сразу, дальше цена идёт за прайсом сама."
      >
        <CatalogGaps />
      </Panel>
