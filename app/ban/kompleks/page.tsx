// English mirror of /ru/zhilye-kompleksy. Same component, just lang='ban'
...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataEn(f, {
    canonicalPath: '/ban/kompleks',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'complexes', locale: 'ban', ...villaCategoryStats((await loadAll()).enriched) })
