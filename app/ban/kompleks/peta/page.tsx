...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataEn(f, { canonicalPath: '/ban/kompleks', noIndex: true })
  meta.title = `Peta · ${buildHeadingLocalized(f, 'ban')} | Balinsky`
  return meta
}
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  const options = buildOptions(enriched, filters, 'ban')
  ...
  return (
    <>
      ...
      <PageContainer>
        <h1 className="pt-8 mb-2 text-[28px] md:text-[36px] font-semibold tracking-tight text-[#111827]">{`Peta · ${buildHeadingLocalized(filters, 'ban')}`}</h1>
        <div className="text-[14px] text-[var(--color-text-muted)] mb-6">{totalPoints} kompleks ring peta{totalPoints !== groups.length && ` · ${groups.length} titik`}</div>

        <CatalogTabs active="map" listHref={buildListHref(filters, 'ban')} mapHref={buildMapHref(filters, 'ban')} lang="ban" />

        <div className="mt-6">
          <ComplexCatalogSearchBar initial={filters.q} current={filters} view="map" placeholder="Rereh kompleks, wilayah, pangwangun…" />
        </div>

        <div className="mt-4">
          <ComplexFiltersBar state={filters} options={options} view="map" lang="ban" />
        </div>

        <div className="mt-6">
          <ComplexesMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="ban" />
        </div>

        <ComplexesSeoContent filters={filters} variant="map" lang="ban" />
