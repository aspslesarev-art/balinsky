...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataLoc(f, 'ban', { canonicalPath: '/ban/vila', noIndex: true })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  for (const e of filtered) {
    ...
    seenSlug.add(slug)
    const titleRaw =
      tField(e.data, 'SEO:Title', 'ban') ?? firstString(e.data['Имя ENG']) ?? firstString(e.data['Name']) ?? slug
  ...
  return (
    <>
      ...
      <PageContainer>
        <h1 className="pt-8 mb-2 text-[28px] md:text-[36px] font-semibold tracking-tight text-[#111827]">{buildHeadingLoc(filters, 'ban')}</h1>
        <div className="text-[14px] text-[var(--color-text-muted)] mb-6">{totalPoints} objek ring peta{totalPoints !== groups.length && ` · ${groups.length} titik`}</div>

        <CatalogTabs active="map" listHref={buildListHref(filters, 'ban')} mapHref={buildMapHref(filters, 'ban')} lang="ban" />

        <div className="mt-6">
          <VillaCatalogSearchBar initial={filters.q} current={filters} view="map" placeholder="Rerehin vila, wewengkon, pangwangun…" />
        </div>

        <div className="mt-4">
          <VillaFiltersBar state={filters} options={options} view="map" lang="ban" />
        </div>

        <div className="mt-6">
          <VillasMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="ban" />
        </div>

        <VillasSeoContent filters={filters} variant="map" lang="ban" />
