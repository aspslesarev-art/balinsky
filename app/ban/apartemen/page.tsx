...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataEn(f, {
    canonicalPath: '/ban/apartemen',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'apartments', locale: 'ban', ...apartmentCategoryStats((await loadAll()).enriched) })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  return <ApartamentyCatalog filters={filters} page={1} basePath="/ban/apartemen" lang="ban" />
}
