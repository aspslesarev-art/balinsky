...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataLoc(f, 'zh', { canonicalPath: '/zh/bieshu', noIndex: true })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  for (const e of filtered) {
    ...
    seenSlug.add(slug)
    const titleRaw =
      tField(e.data, 'SEO:Title', 'zh') ?? firstString(e.data['Имя ENG']) ?? firstString(e.data['Name']) ?? slug
  ...
  return (
    <>
      ...
      <PageContainer>
        ...
        <div className="mt-6">
          <VillaCatalogSearchBar initial={filters.q} current={filters} view="map" placeholder="搜索别墅、地区、开发商…" />
        </div>

        <div className="mt-4">
          <VillaFiltersBar state={filters} options={options} view="map" lang="zh" />
        </div>

        <div className="mt-6">
          <VillasMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="zh" />
        </div>

        <VillasSeoContent filters={filters} variant="map" lang="zh" />
