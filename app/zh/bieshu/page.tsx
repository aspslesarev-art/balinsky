...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataLoc(f, 'zh', {
    canonicalPath: '/zh/bieshu',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  // TASK-13c: commercial "number + price + USP" meta on the bare category root.
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'villas', locale: 'zh', ...villaCategoryStats((await loadAll()).enriched) })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  return <VillasCatalog filters={filters} page={1} basePath="/zh/bieshu" lang="zh" />
}
