// English mirror of /ru/zhilye-kompleksy. Same component, just lang='zh'
...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataEn(f, {
    canonicalPath: '/zh/zhuzhaiqu',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'complexes', locale: 'zh', ...villaCategoryStats((await loadAll()).enriched) })
