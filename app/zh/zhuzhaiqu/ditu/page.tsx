...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataEn(f, { canonicalPath: '/zh/zhuzhaiqu', noIndex: true })
  meta.title = `地图 · ${buildHeadingLocalized(f, 'zh')} | Balinsky`
  return meta
}
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  const options = buildOptions(enriched, filters, 'zh')
  ...
  for (const e of filtered) {
    ...
    seenSlug.add(e.slug)
    const types = e.types.length > 0 ? e.types.map(t => facetLabel('type', t, 'zh')).join(', ') : null
  ...
  return (
    <>
      ...
      <PageContainer>
        ...
        <div className="mt-6">
          <ComplexCatalogSearchBar initial={filters.q} current={filters} view="map" placeholder="搜索住宅区、地区、开发商…" />
        </div>

        <div className="mt-4">
          <ComplexFiltersBar state={filters} options={options} view="map" lang="zh" />
        </div>

        <div className="mt-6">
          <ComplexesMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="zh" />
        </div>

        <ComplexesSeoContent filters={filters} variant="map" lang="zh" />
