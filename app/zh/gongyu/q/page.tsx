...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataLoc(f, 'zh', {
    canonicalPath: '/zh/gongyu',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'apartments', locale: 'zh', ...apartmentCategoryStats((await loadAll()).enriched) })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  const localCanonical = canonical ? hubPath(canonical, 'zh') : null
  if (localCanonical && canonical !== '/ru/apartamenty' && hasAnyFilter(filters)) {
    permanentRedirect(localCanonical)
  }
  return <ApartamentyCatalog filters={filters} page={1} basePath="/zh/gongyu" lang="zh" />
}
