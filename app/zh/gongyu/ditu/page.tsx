...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataLoc(f, 'zh', { canonicalPath: '/zh/gongyu', noIndex: true })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  for (const e of filtered) {
    ...
    seenSlug.add(slug)
    const titleRaw =
      tField(e.data, 'SEO:Title', 'zh') ?? firstString(e.data['Name']) ?? slug
  ...
  return (
    <>
      ...
      <PageContainer>
        <h1 className="pt-8 mb-2 text-[28px] md:text-[36px] font-semibold tracking-tight text-[#1A1A1A]">{buildHeadingLoc(filters, 'zh')}</h1>
        <div className="text-[14px] text-[var(--color-text-muted)] mb-6">
          {pointCount} 个房源在地图上
          {pointCount !== groups.length && ` · ${groups.length} 个坐标点`}
        </div>

        <CatalogTabs active="map" listHref={buildListHref(filters, 'zh')} mapHref={buildMapHref(filters, 'zh')} lang="zh" />

        <div className="mt-6">
          <CatalogSearchBar initial={filters.q} current={filters} view="map" placeholder="按名称、地区、开发商搜索…" />
        </div>

        <div className="mt-4">
          <FiltersBar state={filters} options={options} view="map" lang="zh" />
        </div>

        <div className="mt-6">
          <ApartmentsMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="zh" />
        </div>

        <SeoContent filters={filters} variant="map" lang="zh" />
