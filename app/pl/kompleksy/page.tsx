// English mirror of /ru/zhilye-kompleksy. Same component, just lang='pl'
...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const base = buildMetadataEn(f, {
    canonicalPath: '/pl/kompleksy',
    noIndex: hasAnyFilter(f) && buildCanonicalPath(f) !== null,
  })
  if (!hasAnyFilter(f)) {
    const cat = generateCategoryMeta({ category: 'complexes', locale: 'pl', ...villaCategoryStats((await loadAll()).enriched) })
