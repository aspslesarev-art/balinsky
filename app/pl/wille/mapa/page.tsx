...
export async function generateMetadata({ searchParams }: { searchParams: SP }) {
  ...
  const meta = buildMetadataEn(f, { canonicalPath: '/pl/wille', noIndex: true })
...
export default async function Page({ searchParams }: { searchParams: SP }) {
  ...
  for (const e of filtered) {
    ...
    seenSlug.add(slug)
    const titleRaw =
      tField(e.data, 'SEO:Title', 'pl') ?? firstString(e.data['Имя ENG']) ?? firstString(e.data['Name']) ?? slug
  ...
  return (
    <>
      ...
      <PageContainer>
        ...
        <div className="mt-4">
          <VillaFiltersBar state={filters} options={options} view="map" lang="pl" />
        </div>

        <div className="mt-6">
          <VillasMap apiKey={apiKey} groups={groups} heatCells={heat.cells} heatMax={heat.max} lang="pl" />
        </div>

        <VillasSeoContent filters={filters} variant="map" lang="pl" />
