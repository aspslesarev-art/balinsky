...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const copy = getDistrictCopy(district, 'pl')
  ...
  return {
    ...
    alternates: {
      canonical: `/pl/inwestycje-nieruchomosci-bali/${district}`,
      languages: {
        ru: `${SITE_URL}/ru/investicii/${district}`,
        pl: `${SITE_URL}/pl/inwestycje-nieruchomosci-bali/${district}`,
    ...
    openGraph: {
      ...
      type: 'article',
      url: `/pl/inwestycje-nieruchomosci-bali/${district}`,
    },
...
export default async function Page({ params }: { params: Params }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: copy.name, item: `${SITE_URL}/pl/inwestycje-nieruchomosci-bali/${district}` },
    ],
  }
  ...
  return (
    <>
      ...
      <PageContainer>
        <Breadcrumbs items={[
          { label: 'Home', href: '/pl' },
          { label: 'Bali property investment', href: '/pl/inwestycje-nieruchomosci-bali' },
          { label: copy.name },
        ]} />
        ...
        <article className="mt-6 mb-16 max-w-4xl">
          ...
          <section className="mb-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Link href={`/pl/wille/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/pl/apartamenty/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
              ...
              <Link href={`/pl/kompleksy/${district}`} className="block rounded-2xl border border-[var(--color-border)] p-5 bg-white no-underline hover:border-[var(--color-primary)] transition-colors">
          ...
          <section className="mb-10 flex items-center gap-2 text-[14px] text-[var(--color-text-muted)]">
            <MapPin size={14} />
            See the district on map — <Link href={`/pl/wille/${district}/karta`} className="text-[var(--color-primary)] no-underline hover:underline">villa map</Link>
