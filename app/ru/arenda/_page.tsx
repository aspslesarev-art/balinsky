...
const COPY = {
  ...
  pl: {
    ...
    h1: 'Wynajmij mieszkanie na Bali',
...
export async function RentalListShell({ initial, lang }: {
  ...
  const c = pickCopy(COPY, lang)
  const isBareHub = initial.districts.length === 0
    && initial.bedrooms.length === 0
  ...
  return (
    <>
      ...
      <PageContainer>
        ...
        {/* Long-form copy only on the bare hub — filtered views are query-param
            URLs (robots disallows `/*?`), so they neither need nor should
            duplicate it. Same rule as the villa/apartment hubs. */}
        {isBareHub && <RentalSeoContent lang={lang} />}
