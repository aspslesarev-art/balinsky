...
const COPY = {
  ...
  de: {
    sort: { newest: 'Neueste zuerst', 'price-asc': 'Preis ↑', 'price-desc': 'Preis ↓', 'br-asc': 'Schlafzimmer ↑', 'br-desc': 'Schlafzimmer ↓' } as Record<SortKey, string>,
    presets: ['Bis $1000', '$1000–2000', '$2000–3000', '$3000+'] as const,
    district: 'Gebiet', bedrooms: 'Schlafzimmer', price: 'Preis', sortLabel: 'Sortierung',
    reset: 'Zurücksetzen', clear: 'Löschen', search: 'Suchen…', noResults: 'Nichts gefunden',
    priceMonthHeader: 'Monatspreis, USD', priceFrom: 'Von', priceTo: 'Bis',
    objects: (n: number) => `${n} ${n === 1 ? 'Objekt' : 'Objekte'}`,
    showingOfTotal: (shown: number, total: number) => `${shown} von ${total} angezeigt — mehr laden`,
    emptyByFilters: 'Keine Objekte für die gewählten Filter. Filter zurücksetzen oder andere ausprobieren.',
    perMonth: '/ Mon.',
    bali: 'Bali',
  },
  zh: {
    sort: { newest: '最新优先', 'price-asc': '价格 ↑', 'price-desc': '价格 ↓', 'br-asc': '卧室 ↑', 'br-desc': '卧室 ↓' } as Record<SortKey, string>,
    presets: ['$1000 以下', '$1000–2000', '$2000–3000', '$3000+'] as const,
    district: '地区', bedrooms: '卧室', price: '价格', sortLabel: '排序',
    reset: '清除', clear: '清除', search: '搜索…', noResults: '没有找到',
    priceMonthHeader: '月租价格，USD', priceFrom: '从', priceTo: '至',
    objects: (n: number) => `${n} 套`,
    showingOfTotal: (shown: number, total: number) => `显示 ${shown} / ${total} — 加载更多`,
    emptyByFilters: '没有符合所选筛选条件的房源。请清除筛选或尝试其他条件。',
    perMonth: '/ 月',
    bali: '巴厘岛',
  },
  nl: {
    sort: { newest: 'Nieuwste eerst', 'price-asc': 'Prijs ↑', 'price-desc': 'Prijs ↓', 'br-asc': 'Slaapkamers ↑', 'br-desc': 'Slaapkamers ↓' } as Record<SortKey, string>,
    presets: ['Tot $1000', '$1000–2000', '$2000–3000', '$3000+'] as const,
    district: 'Gebied', bedrooms: 'Slaapkamers', price: 'Prijs', sortLabel: 'Sorteren',
    reset: 'Wissen', clear: 'Wissen', search: 'Zoeken…', noResults: 'Niets gevonden',
    priceMonthHeader: 'Maandprijs, USD', priceFrom: 'Van', priceTo: 'Tot',
    objects: (n: number) => `${n} ${n === 1 ? 'woning' : 'woningen'}`,
    showingOfTotal: (shown: number, total: number) => `${shown} van ${total} weergegeven — meer laden`,
    emptyByFilters: 'Geen woningen voldoen aan de geselecteerde filters. Wis de filters of probeer andere.',
    perMonth: '/ mnd',
    bali: 'Bali',
  },
  ban: {
    sort: { newest: 'Sané anyar dumun', 'price-asc': 'Aji ↑', 'price-desc': 'Aji ↓', 'br-asc': 'Kamar ↑', 'br-desc': 'Kamar ↓' } as Record<SortKey, string>,
    presets: ['Ngantos $1000', '$1000–2000', '$2000–3000', '$3000+'] as const,
    district: 'Wewidangan', bedrooms: 'Kamar', price: 'Aji', sortLabel: 'Urut',
    reset: 'Kosongin', clear: 'Kosongin', search: 'Rereh…', noResults: 'Nénten wénten asil',
    priceMonthHeader: 'Aji sabulan, USD', priceFrom: 'Saking', priceTo: 'Nganti',
    objects: (n: number) => `${n} unit`,
    showingOfTotal: (shown: number, total: number) => `Ngedeng ${shown} saking ${total} — muat malih`,
    emptyByFilters: 'Nénten wénten unit sané cocok sareng saringan. Kosongin saringan utawi coba sané lianan.',
    perMonth: '/ bulan',
    bali: 'Bali',
  },
} as const
