...
export async function loadJson<T>(url: string, fallback: T, tags?: string[]): Promise<T> {
  try {
...
export function buildOptions(
  ...
  const DEAL_LABELS: Record<Lang, Record<DealType, string>> = {
    ...
    de: { primary: 'Vom Bauträger',  resale: 'Weiterverkauf', secondary: 'Bestand' },
...
async function _loadAllInternal(): Promise<CachedAll> {
  const [rowsRes, manifestRaw, devMap, enCache, viewCounts, featuresMap, visionMap] = await Promise.all([
    ...
    loadJson<Record<string, string[]>>(cdnManifestUrl(PHOTO_MANIFEST_URL, 600), {}, ['content:apartments']),
    loadJson<Record<string, string>>(cdnManifestUrl(DEV_LOOKUP_URL, 600), {}, ['content:apartments']),
...
// getDistrictCommercialMeta is ru/en-only, so we compose the title/description
// straight from the localized heading — no English on filtered non-RU combos.
const APT_DESC_TAIL: Record<Exclude<Lang, 'ru' | 'en'>, string> = {
...
export function buildDescriptionLoc(f: FilterState, lang: Lang, totalCount?: number): string {
  ...
  return lang === 'zh' ? `${heading}。${APT_DESC_TAIL[lang]}` : `${heading}. ${APT_DESC_TAIL[lang]}`
}
