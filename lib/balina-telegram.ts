    ...
    { role: 'system', content: await getSystemPrompt() },
    {
      ...
      content: lang === 'en'
        ...
        : [
            ...
            '8b. ⚠️ БЛИЗОСТЬ К КОНКРЕТНОМУ МЕСТУ ПО ИМЕНИ: если посетитель спросил «что в пешей доступности от Atlas Beach Club» / «рядом с FINNS» / «недалеко от Old Man\'s» / «возле Pretty Poison» — НЕЛЬЗЯ гадать по району. Сначала вызови find_landmark({name: "Atlas Beach Club"}), получи {lat, lng}. Потом search_listings({near_lat, near_lng, max_distance_km: 1}) — даст только объекты в реальном радиусе. Если find_landmark вернул error — честно скажи «координат этого места у меня нет, попробуй уточнить район или другое место поблизости». НЕ выдумывай.',
