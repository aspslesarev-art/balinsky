...
export async function replyAsBalina({
  ...
  try {
    return await runTurn({ chatId, token, lang, userText, voiceFileId })
  } finally {
    stopTyping()
  }
}
...
async function runTurn(
  ...
  if (!textIn && voiceFileId) {
    const transcribeClient = buildTranscribeClient()
    const transcript = transcribeClient
      ? await transcribeVoice(transcribeClient, token, voiceFileId).catch(err => {
  ...
  if (correction) {
    if (!isOwnerChat(chatId)) {
      await sendText(token, chatId,
        '🔒 Обучение Андрея доступно только владельцу. Ваше сообщение я обработаю как обычный запрос — пишите, что ищете.')
    ...
    { role: 'system', content: await getSystemPrompt() },
    {
      ...
      content: lang !== 'ru'
        ...
        : [
            ...
            '5e. ⚠️ ФРИХОЛД / ХГБ: если посетитель просит «фрихолд», «freehold», «полное право собственности», «ХГБ», «HGB», «Hak Milik» — НЕ ИЩИ объекты. Сразу честно ответь:',
            '     «Иностранцу в Индонезии freehold (Hak Milik) недоступен по закону — этим правом владеют только граждане Индонезии. Для иностранцев существуют две легальные схемы:',
            '     • <b>Leasehold</b> (Hak Sewa) — долгосрочная аренда земли на 25–80 лет с правом продления. 90% сделок на Бали — именно так.',
            '     • <b>Hak Pakai через PT PMA</b> — оформляешь индонезийское юрлицо и оно владеет правом пользования. Сложнее, ~$3-5k на регистрацию + ежегодная отчётность.',
            '     ХГБ (Hak Guna Bangunan) — тоже только для граждан Индонезии или PT PMA. Прямого ХГБ-владения для иностранца нет.',
            '     В нашем каталоге — только leasehold-объекты. Если интересна схема через PT PMA — подключу юриста, который оформит.»',
            '     Если посетитель после этого согласен на leasehold — продолжай поиск как обычно. НЕ выдумывай freehold-объекты.',
            ...
            '8b. ⚠️ БЛИЗОСТЬ К КОНКРЕТНОМУ МЕСТУ ПО ИМЕНИ: если посетитель спросил «что в пешей доступности от Atlas Beach Club» / «рядом с FINNS» / «недалеко от Old Man\'s» / «возле Pretty Poison» — НЕЛЬЗЯ гадать по району. Сначала вызови find_landmark({name: "Atlas Beach Club"}), получи {lat, lng, district}. Потом ВСЕГДА search_listings({near_lat, near_lng, max_distance_km: 1.5, district: "<district из ответа find_landmark>"}). district обязателен — у многих объектов в каталоге нет geo, и без district fallback они выпадают, оставляя 0 результатов. Если в первом проходе всё равно 0 — повтори БЕЗ near_lat/near_lng, только с district (берёт весь район как ближайший proxy). Если find_landmark вернул error — честно скажи «координат этого места у меня нет, уточни район или название поблизости». НЕ выдумывай.',
            ...
            '10. СРАВНЕНИЕ ДВУХ И БОЛЕЕ ОБЪЕКТОВ — отдельный шаблон, не общий «вот варианты»:',
            '   - Сначала найди оба объекта через search_listings({query: "X"}) и search_listings({query: "Y"}). Для глубоких параметров (готовность, амбиции отделки, рассрочка) — get_listing_full по каждому.',
            '   - Сравни по 4-6 ключевым параметрам где они РАЗЛИЧАЮТСЯ. Бери из карточек: цена, цена/м², площадь, спальни, лизхолд (года), земля (land_zone), расстояние до пляжа (distance_to_beach_km), статус готовности, cap_rate_good, район.',
            '   - Формат ответа (точно по шаблону, ничего лишнего):',
            '     Сравнение: <Имя X> vs <Имя Y>',
            '     ',
            '     🟢 <X> выигрывает:',
            '     • <критерий 1> (значение_X vs значение_Y)',
            '     • <критерий 2> (...)',
            '     ',
            '     🔴 <Y> выигрывает:',
            '     • <критерий 3> (...)',
            '     • <критерий 4> (...)',
            '     ',
            '     🤝 Равны/неважно: <критерии где разница пренебрежимо мала, одной строкой>.',
            '     ',
            '     Вывод: для <цели A> — <X>, потому что <главный аргумент>. Для <цели B> — <Y>, потому что <главный аргумент>.',
            '   - НЕ описывай каждый объект отдельной простыней — это и так видно на карточках. Только бок-о-бок различия.',
            '   - Если разница в каком-то параметре <10 % — не выноси это в победу, это шум.',
            '   - После шаблона приди к выводу с РЕКОМЕНДАЦИЕЙ под цели «жить с семьёй», «инвестировать под Booking», «сдавать помесячно» — выбери релевантную если ясна, иначе все три.',
            '   - В конце: 2 карточки (по одной на объект) — уйдут отдельными сообщениями.',
            '',
...
function looksLikeListingRequest(text: string): boolean {
  ...
  // Single short message that's mostly a Latin proper noun
  // (e.g. "Surfside", "Origins", "Maison Boheme") — almost always
  // a property-by-name lookup, force the tool call so the model
  // doesn't reply from memory.
  const trimmed = text.trim()
  if (trimmed.length <= 40) {
    const looksLikeName = /^[A-Z][a-zA-Z]{2,}([\s'-][A-Z][a-zA-Z]+){0,3}\??$/.test(trimmed)
...
function formatCaption(card: ListingCard, lang: Lang): string {
  ...
  if (card.price_per_sqm_usd != null) facts.push(`$${Math.round(card.price_per_sqm_usd).toLocaleString('en-US')}/м²`)
