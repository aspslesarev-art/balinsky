...
export type CollectionConfig = {
  ...
  /** Ключ внутри `data`, который обязан повторять slug-КОЛОНКУ. У комплексов
   *  сайт resolve'ит страницу по колонке `slug`, а база знаний ассистента и
   *  связка с трекером читают `SEO:Slug` из `data`; строка без него для них
   *  не существует. Заполняется и чинится автоматически при записи. */
  mirrorSlugField?: string
