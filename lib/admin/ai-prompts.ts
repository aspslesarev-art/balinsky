...
const TASKS: Partial<Record<AiTask, (ctx: string) => string>> = {
  ...
  search_variants: ctx =>
    `Перечисли варианты написания названия, по которым эту запись может искать человек или матчить парсер.\n` +
    `Требования: латиница И кириллица; с артиклями и без; сокращения и расшифровки; дефис/пробел/слитно; ` +
    `частые опечатки и транслит; БЕЗ выдуманных других объектов. ` +
    `Верни одной строкой через запятую, 5–15 вариантов, без нумерации и кавычек.\n\nДанные:\n${ctx}`,
  reputation: ctx =>
