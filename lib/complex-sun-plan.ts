...
export type SitePlan = {
  ...
  basemap: {
    ...
    /**
     * Обзорные снимки пошире, под основным, от ближнего к дальнему: основной
     * держит резкость у комплекса, обзорные показывают округу (у EDEM II — до
     * океана). Якорь — тот же смысл, что у основного: пиксель центра участка.
     * Ползунки «Снимок» двигают все слои вместе.
     */
    wide?: {
      url: string
      sizePx: number
...
const EDEM_II: SitePlan = {
  ...
  basemap: {
    ...
    //
    // Под ним — ≈ 4 × 4 км, зум 18 (6784 px → 4096, ~1 м/px), центр сдвинут
    // на 600 м к югу: так в кадр входит всё побережье, а не угол океана.
    wide: [
      {
      ...
      {
        url: '/sun/edem-ii-region.jpg',
        sizePx: 4096,
        metersPerPixel: 0.97733,
        anchorPx: { x: 2100.3, y: 1476.2 },
      },
    ],
  },
}
