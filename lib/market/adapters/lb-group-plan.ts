// Разбор генпланов LB Group.
//
// У LB прайс — не таблица, а карта посёлка: юниты («№34, 1br») разложены
// по клеткам в форме застройки, статус закодирован заливкой по легенде
// Free/Booked/Sold/Resale/Block, а цена лежит отдельным блоком по типу
// юнита. Отсюда четыре раскладки ниже: апартаменты, виллы, «голые числа
// в клетках» и буквенно-цифровые ID — extractLbUnits пробует все и берёт
// ту, что дала больше юнитов.
//
// Код достался от прежней системы парсеров (она писала в Airtable и
// удалена); здесь оставлен только разбор, без записи куда-либо.

import { colorDistance, hexToRgb, type XlsxSheet } from '../xlsx'

// Числа в генплане пишут по-разному: «215 000$», «1,549», «45.00 m2».
// Функция переехала сюда из общего модуля старых парсеров вместе с
// разбором — чтобы поведение проверенного парсера LB не поехало.
