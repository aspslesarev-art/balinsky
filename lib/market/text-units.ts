...
Верни ТОЛЬКО JSON:
{
  ...
  "unsupported": "<если на странице нет ни одного объекта — одна фраза почему, иначе null. Отсутствие цен само по себе НЕ повод: юниты с номерами и статусами возвращай>"
}
...
- Юниты без цен — это нормальный ответ, а не unsupported: если у объектов есть свои номера и виден статус продажи (SOLD/AVAILABLE/продан/свободен), верни их все с priceUsd = null. Историю «продано/свободно» мы ведём и без прайса.
