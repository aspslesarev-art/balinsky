...
// filters. Tracking parameters (utm_*, gclid, fbclid …) are not filters: ad
// traffic keeps the cached page. /q is internal; a direct hit is sent to the
// root with its query, which lands back here and is rewritten.
...
const TRACKING_PARAM = /^(utm_|fbclid$|gclid$|gbraid$|wbraid$|yclid$|_ga$|_gl$|mc_|srsltid$|ref$)/

function handleCatalogQuery(req: NextRequest): NextResponse | null {
  const path = req.nextUrl.pathname.replace(/\/$/, '')
  if (path.endsWith('/q') && CATALOG_ROOTS.has(path.slice(0, -2))) {
    const url = req.nextUrl.clone()
    url.pathname = path.slice(0, -2)
    return NextResponse.redirect(url, 308)
  }
  if (!CATALOG_ROOTS.has(path)) return null
  const hasFilters = [...req.nextUrl.searchParams.keys()].some(k => !TRACKING_PARAM.test(k))
  if (!hasFilters) return NextResponse.next()
  const url = req.nextUrl.clone()
  url.pathname = `${path}/q`
...
export async function middleware(req: NextRequest) {
  ...
  const catalog = handleCatalogQuery(req)
