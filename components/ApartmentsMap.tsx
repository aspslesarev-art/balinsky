...
// One pin on the map. Multiple apartments may share the same coordinates
// (different units in the same building) — they're grouped here so the user
// can see all options under one marker.
...
// the marker represents multiple co-located apartments.
function pinSvg({
  bg,
  size = 32,
  ring,
  ...
  const inner =
    count > 1
      ? `<text x="${half}" y="${half}" text-anchor="middle" dominant-baseline="central"
              ...
              font-size="${count >= 100 ? size * 0.32 : size * 0.4}" font-weight="700" fill="${COLORS.white}">${count}</text>`
...
function makeIcon(opts: { count: number; selected?: boolean; hover?: boolean }) {
  const sizeBase = opts.count > 1 ? 36 : 32
  const size = opts.selected ? sizeBase + 6 : opts.hover ? sizeBase + 4 : sizeBase
...
function MapMarkers({
  ...
  // Stable key reflecting both group identity AND its size, so the marker
  // count badge updates when filters add/remove items in a group.
  const groupsKey = groups.map(g => `${g.key}:${g.items.length}`).join('|')
  ...
  useEffect(() => {
    ...
    for (const g of groups) {
      ...
      marker.addListener('mouseover', () => {
        if (g.key !== selectedKey) marker.setIcon(makeIcon({ count, hover: true }))
      })
  ...
  // Selected icon swap (independent of marker rebuilding).
  useEffect(() => {
    ...
    for (const [key, marker] of m.entries()) {
      ...
      if (!g) continue
      marker.setIcon(makeIcon({ count: g.items.length, selected: key === selectedKey }))
